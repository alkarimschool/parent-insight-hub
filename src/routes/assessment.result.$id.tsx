...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-soft">
      <h3 className="flex items-center gap-2 text-lg font-semibold text-foreground"><Sparkles className="h-4 w-4 text-primary" />{title}</h3>
...
function List({ items }: { items?: string[] }) {
  ...
  return (
    <ul className="space-y-2">
      {items.map((it, i) => (
        <li key={i} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{it}</span></li>
      ))}
    </ul>
  );
}
  ...
  const result = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("ai_results")
        .select("content, created_at, assessments(child_id, children(name, birth_date, gender), parents(name))")
        .eq("assessment_id", id)
        .maybeSingle();
      return data;
    },
    ...
    <div className="min-h-screen bg-gradient-soft">
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <div className="mb-8 text-center">
          <div className="mx-auto inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-xs font-semibold text-primary">Laporan Assessment</div>
          <h1 className="mt-4 text-3xl font-bold text-foreground sm:text-4xl">Hasil untuk {childName || "Anak Anda"}</h1>
          <p className="mt-2 text-muted-foreground">Hasil AI ini adalah rekomendasi awal, bukan diagnosis medis.</p>
        </div>

        {!c ? (
          <div className="rounded-3xl border border-border/60 bg-card p-10 text-center shadow-soft">
            <div className="mx-auto h-10 w-10 animate-spin rounded-full border-2 border-primary border-t-transparent" />
            <p className="mt-4 text-sm text-muted-foreground">AI sedang menganalisis jawaban Anda…</p>
          </div>
        ) : (
          <div className="grid gap-5">
            <Section title="1. Ringkasan Perkembangan Anak"><p>{c.ringkasan}</p></Section>
            ...
            <Section title="4. Kecerdasan Sosial"><p>{c.kecerdasan_sosial}</p></Section>
            <Section title="5. Kecerdasan Emosional"><p>{c.kecerdasan_emosional}</p></Section>
            ...
            <div className="mt-4 flex justify-center gap-3">
              ...
              <button onClick={() => window.print()} className="rounded-full bg-gradient-hero px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-soft">Cetak Laporan</button>
            </div>
          </div>
        )}
      </div>
