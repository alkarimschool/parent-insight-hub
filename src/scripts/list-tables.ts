...
async function listTables() {
  ...
  const { data: appSettings, error: sErr } = await supabaseAdmin.from("app_settings").select("*");
  console.log("app_settings:", sErr ? sErr.message : appSettings);

  const { data: systemSettings, error: sysErr } = await supabaseAdmin.from("system_settings").select("*");
